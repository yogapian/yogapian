...
- 에러 발생 시 초보자도 이해할 수 있는 한글 비유로 먼저 설명
...
### 예약
- 출석 처리 시에만 1회 차감. 예약·노쇼·취소는 차감 없음
- 잔여 1회: "다음 예약 시 갱신이 필요합니다" 팝업 → 예약 진행
- 잔여 0회 또는 기간 만료: 갱신 안내 팝업 → 선택 시 booking에 `renewalPending:true` 플래그로 임시 1회 생성 → 예약 완료
- 임시 1회는 관리자가 갱신 처리 시 수동으로 "임시 1회 포함" 체크박스로 새 회원권에 포함
- 휴강일·과거 날짜·시작된 회차 선택 불가
- 같은 날·같은 회차 중복 예약 불가
- 대기: 달력에 ▲ 표시, 클릭 시 해당 회차에 "대기 N번째 · 대기취소" 표시

### 회원 상태 탭
- ON / OFF / HOLD / RENEW 4개 탭
- RENEW 조건: 잔여 0회 OR 기간 만료 OR `renewalPending:true` 상태
- 관리자 로그인 시 오늘 임시예약(`renewalPending`) 회원 팝업 알림
- 출석보드에서 해당 회원 이름 옆 "갱신" 표기

### 홀딩
- 3개월권만 가능, 3개월권 당 1회 제한, 1개월권 불가
- 시작: 관리자 설정 / 해제: 회원이 배너에서 직접 "복귀하기"
- 홀딩 기간 중 예약 불가
- 홀딩 일수만큼 종료일 자동 연장
...
- 관리자 출석 처리 → 1회 차감
- 회원 취소·노쇼·대기취소 → 차감 없음
- `usedAsOf()` 함수로만 잔여 계산. `member.used` 직접 조작 금지

### 정원
수업별 `capacity` 필드로 관리. `getSlotCapacity(date, slotKey, specialSchedules, scheduleTemplate)` 사용. 전역 `SLOT_LIMIT` 상수 사용 금지.

### 원데이
- 전용 링크 → 가입 → 시간표 선택 → 30,000원 결제
- 3일 이내 정회원 전환 시 30,000원 차감
- 3일 초과 시 데이터 자동 삭제
- 환불: 2일전 100% / 1일전 50% / 당일 0% → 관리자 수동 처리

### 오픈 클래스
회원 횟수 차감 없음. 별도 신청, 무료 또는 유료.
