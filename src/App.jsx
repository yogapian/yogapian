...
function AdminApp({members,setMembers,bookings,setBookings,notices,setNotices,specialSchedules,setSpecialSchedules,closures,setClosures,onLogout}){
  ...
  function applyHolding(mid,hd){setMembers(p=>p.map(m=>{if(m.id!==mid)return m;if(!hd)return{...m,holding:null,holdingDays:0};// 복귀: holdingDays에 경과일 반영, holding 해제
if(hd.resumed)return{...m,holding:null,holdingDays:0,extensionDays:(m.extensionDays||0)+hd.workdays};// 홀딩 시작: endDate 미정, holdingDays는 복귀 시 계산
