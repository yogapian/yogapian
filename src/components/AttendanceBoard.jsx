...
export default function AttendanceBoard({members,bookings,setBookings,setMembers,specialSchedules,setSpecialSchedules,closures,setClosures,notices,setNotices,scheduleTemplate,setScheduleTemplate,onMemberClick,onRefresh}){
  ...
  const INIT_SP={date:TODAY_STR,label:"",type:"regular",feeNote:"",dailyNote:"",activeSlots:[],customTimes:{dawn:"06:30",morning:"08:30",lunch:"11:50",afternoon:"14:00",evening:"19:30"},slotCapacity:{}};
  ...
  function adminCancel(id, note, sendNotice=true){
    ...
    if(sendNotice && b.memberId){
      ...
      const nid1 = Date.now();
      setNotices(prev=>[{id:nid1, title:`❌예약취소❌`, content:`${fmtWithDow(b.date)}\n수업 예약이 취소되었습니다.${note?" ("+note+")":""}`, pinned:false, createdAt:TODAY_STR, targetMemberId:b.memberId}, ...prev]);
    }
    if(firstWaiter){
                    ...
                    return(
                        <div key={rec.id} draggable={!slotCl&&!isWaiting} onDragStart={e=>!slotCl&&!isWaiting&&onDragStart(e,rec.id)} onDragEnd={onDragEnd}
                          ...
                          <div style={{flex:1,minWidth:0,display:"flex",alignItems:"center",gap:4,overflow:"hidden"}}>
                            ...
                            {/* 갱신 뱃지 + 💳 아이콘: renewalPending 예약(갱신·결제대기 포함) */}
                            {!isOneday&&rec.renewalPending&&<><span style={{fontSize:10,background:"#fdf3e3",color:"#9a5a10",borderRadius:5,padding:"1px 6px",fontWeight:700,flexShrink:0}}>갱신</span><span style={{fontSize:13,flexShrink:0}}>💳</span></>}
                            {/* paymentPending이지만 renewalPending 예약 없을 때 💳만 표시 (기존 예약 호환) */}
                            {!isOneday&&mem?.paymentPending&&!rec.renewalPending&&<span style={{fontSize:13,flexShrink:0}}>💳</span>}
            ...
            <div style={S.modalBtns}>
              ...
              <button style={{...S.saveBtn,background:"#8e3030",opacity:closureForm.reason?1:0.5}} disabled={!closureForm.reason} onClick={()=>{
                ...
                if(isExtra){
                  ...
                  const autoNotice={
                    id:noticeId,
                    title:`${fmtWithDow(closureForm.date)} ${typeLabel} 안내`,
            ...
            <div style={{display:"flex",gap:8}}>
              <button style={{flex:1,background:"#f0ece4",color:"#c97474",border:"1px solid #e8c0c0",borderRadius:9,padding:"11px 0",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:FONT}}
                onClick={()=>{
                  ...
                  if(waitPopup.mem) setNotices(prev=>[{id:nid,title:`❌대기취소❌`,content:`${fmtWithDow(date)}\n수업 대기가 취소되었습니다.`,pinned:false,createdAt:TODAY_STR,targetMemberId:waitPopup.mem.id},...(prev||[])]);
                  setWaitPopup(null);
                }}>거절</button>
              <button style={{flex:1,background:"#4a6a4a",color:"#fff",border:"none",borderRadius:9,padding:"11px 0",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:FONT}}
                onClick={()=>{
                  ...
                  if(waitPopup.mem) setNotices(prev=>[{id:nid,title:`✅예약확정✅`,content:`${fmtWithDow(date)}\n수업 예약이 확정되었습니다.`,pinned:false,createdAt:TODAY_STR,targetMemberId:waitPopup.mem.id},...(prev||[])]);
                  setWaitPopup(null);
                }}>수락</button>
            </div>
